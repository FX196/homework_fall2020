...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        next_obs_pred = obs_unnormalized + unnormalize(ptu.to_numpy(delta_pred_normalized), obs_mean, obs_std)  # TODO(Q1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        loss = self.loss(self.forward(observations, actions, **data_statistics)[1], target)  # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
